...
CBullet *CBullet::Create(D3DXVECTOR3 pos, D3DXVECTOR3 rot, D3DXVECTOR3 move,BulletType btype,TYPE type)
{
	...
	if (pBullet == NULL)
	{
		switch (btype)
		{
		case CBullet::TYPE_A:		//プレイヤー

			pBullet = new CBulletA;

			break;

		case CBullet::TYPE_B:		//プレイヤー

			pBullet = new CBulletB;

			break;

		case CBullet::TYPE_C:		//敵
	...
	for (int nCntModel = 0; nCntModel < MAX_OBJECT; nCntModel++)
	{
		...
		{//メモリが使用されているとき
		 ...
		 //種類を取得
			...
			{
				if (type == pObj->TYPE_MODEL || type == TYPE_ENEMY || type == pObj->TYPE_COA)
				{//種類がモデルの場合
