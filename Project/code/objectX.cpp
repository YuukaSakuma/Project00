	...
	for (int nCntModel = 0; nCntModel < MAX_OBJECT; nCntModel++)
	{
		...
		{//メモリが使用されているとき
		 ...
		 //種類を取得
			...
			if (type == pObj->TYPE_MODEL || type == TYPE_ITEM || type == TYPE_COA)
			{//種類がモデルの場合
...
//HRESULT CObjectX::Init(void)
//{
//	LPDIRECT3DDEVICE9 pDevice = CManager::Get()->GetRenderer()->GetDevice();		//デバイスへのポインタを取得
//
//																			//各種変数の初期化
...
//	CXFile *pModelFile = CManager::Get()->GetModelFile();	// Xファイル情報のポインタ
...
//
//			//テクスチャの設定
//			pDevice->SetTexture(0, pTexture->GetAddress(pFileData->pIdexTexture[nCntMat]));
//
//			//モデル(パーツ)の描画
...
//// 生成
////==========================================================
//CObjectX *CObjectX::Create(D3DXVECTOR3 pos, D3DXVECTOR3 rot, const char *pFileName)
//{
//	CObjectX *pObjectX = NULL;
//	CXFile *pModelFile = CManager::Get()->GetModelFile();
//
//	// オブジェクトXの生成
//	pObjectX = new CObjectX();
//
